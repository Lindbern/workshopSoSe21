...
//Konstanten
const SQL_INSERT='INSERT INTO Impfstoff (_Impftermin, _Impfstoffart, `Bezeichnung` , `Hersteller` , `Zulassungsdatum` , `Zulassungsstelle` ) VALUES (?, ?, ?, ?, ?, ?)';
const SQL_UPDATE='UPDATE Impfstoff SET _Impftermin=?, _Impfstoffart=?, `Bezeichnung` =?, `Hersteller` =?, `Zulassungsdatum` =?, `Zulassungsstelle` =? where id=?';
const SQL_SELECT_PK='SELECT Impfstoff.`c_ts` as `c_ts`, Impfstoff.`m_ts` as `m_ts`, Impfstoff.`id` as `id`, Impfstoff._Impftermin as _Impftermin, Impfstoff._Impfstoffart as _Impfstoffart, `Impfstoff`.`Bezeichnung` as `Bezeichnung`, `Impfstoff`.`Hersteller` as `Hersteller`, `Impfstoff`.`Zulassungsdatum` as `Zulassungsdatum`, `Impfstoff`.`Zulassungsstelle` as `Zulassungsstelle` from Impfstoff where Impfstoff.`id` = ?';
...
const SQL_SELECT_Impftermin='select Impfstoff.id as id, Impfstoff.Bezeichnung as Bezeichnung, Impfstoff.Hersteller as Hersteller, Impfstoff.Zulassungsdatum as Zulassungsdatum, Impfstoff.Zulassungsstelle as Zulassungsstelle from Impfstoff where Impfstoff._Impftermin = ?';
